...
import { TrendingUp, Mail, Lock, User, UserPlus, AlertCircle, Check, X, Eye, EyeOff } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { validateSignup, scorePassword, validateEmail } from '../utils/validation';

// OAuth redirects hop through the backend — use the vite-proxied /api/v1 prefix.
...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-20 px-4 md:px-6 pb-12 flex items-center justify-center">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={submit} noValidate className="bg-white border border-slate-200 rounded-xl p-7 space-y-4 shadow-card">
          ...
          <div>
            <label className="text-xs text-slate-600 font-medium mb-1 block">Full Name</label>
          ...
          <div>
            <label className="text-xs text-slate-600 font-medium mb-1 block">Confirm Password</label>
            <div className={`flex items-center gap-2 bg-slate-50 border rounded-lg px-3.5 py-2.5 transition ${confirmErr ? 'border-red-300' : 'border-slate-200 focus-within:border-blue-400'}`}>
              <Lock size={15} className="text-slate-400" />
            ...
            {confirmErr && <div className="text-[11px] text-red-600 mt-1 flex items-center gap-1"><AlertCircle size={11} /> {confirmErr}</div>}
          </div>
